...
```{r,Q1.2 - Exploring apt_buildings}
...
**The apt buildings dataset is a tibble dataframe. It has 37 columns, 1 being the apartment ID and the other 36 being variables such as whether there are amenities, balconies, bike parking, and garbage chutes. Within the variables, the data types are numeric, character, and logical data types, indicated by *\<dbl\>, \<chr\>, and \<lgl\>*, respectively.**

**Next is the flow_sample dataset:**

```{r, Q 1.2 - Exploring flow_sample}
# what is the class type of this dataset?
class(flow_sample)
...
**flow_sample is a tibble dataframe. The variables are "station_id", "year", "extreme_type", "month", "data", "flow", and "sym." It includes numeric and character data types.**

**Next, steam_games:**

```{r, Q 1.2 - Exploring steam_games}
# what is the class type of this dataset?
class(steam_games)
...
**The dataset steam_games has the class of spec_tbl_df, a subclass of a tibble dataframe. There are 21 columns, the first being "id" and the other 20 being variables such as the name of the game, language, publisher, genre, original price, and discounted price. It includes numeric and character data types.**

**Finally, vancouver_trees:**

```{r, Q 1.2 - Exploring vancouver_trees}
# what is the class type of this dataset?
...
**vancouver_trees is a tibble data frame. It has 20 columns, the first being the tree ID and the following being variables such as the tree species, neighbourhood of which it was planted, its common name, street name, date planted, and its longtitude and latitude coordinates. It has numeric, character, and date data types.**
...
**I am interested in using the vancouver_trees dataset. I think it's an interesting dataset because it has a broad range of information, from the species name to the exact location the tree is planted. The sample size is also very large, so there is a lot of data to work with.**
...
**A question I can ask with this data set is: What is the relationship, if any, between tree species and the location they are planted in?**
...
**The first task I will complete is taking a look at how many missing observations there are in each variable of the dataset. By doing so, I can assess if there variables that might not have enough information for downstream analyses.**

```{r, Q 2.1.1 - Determining the percentage of NA observations per variable}
...
**The plot data frame produced plus the plot shows that there are 5 variables that have missing observations: date planted (52%), cultivar name (46%), latitude (16%), longitude (16%), and plant area (1%).**

**The second task I will complete is subsetting the dataframe to have variables and observations that I might be interested in exploring for the next tasks: tree ID, species name, common name of the tree species, the name of the neighbourhood the tree was planted in, the diameter of the trunk, whether the tree was planted on the curb, and the date the tree was planted on**.

```{r, Q 2.1.2 - Subsetting data}
...
**The third task will be determining how many trees are in each neighbourhood. This could tell us which neighbourhoods has the most or least trees and we can begin to see if there are any trends we can explore further later on.**

```{r, Q 2.1.3 - Frequency of trees per neighbourhood}
...
**From the plot, we can see that the neighbourhood with the most trees is Renfrew-Collingwood, followed closely by Kensington-Cedar Cottage, and the neighbourhood with the least trees is Strathcona.**

**For the last task, I will explore the relationship between tree genus and diameter. This could give us information about the range of diameters for each genus. Again, this may identify any trends I can look at further later on, plus maybe identify any possible outliers.**

```{r, Q 2.1.4 - Plotting genus and diameter of trunk}
...
**The first plot, with all the available dater for diameter, shows a few points that may be outliers. For example, the datapoint showing the largest diameter is tree ID 199599. It has a recorded diameter of "435.00" and because no units were provided, I looked for what each variable meant on the [Vancouver Open Data Portal](https://opendata.vancouver.ca/explore/dataset/street-trees/information/?disjunctive.species_name&disjunctive.common_name&disjunctive.on_street&disjunctive.neighbourhood_name) website. For "diameter," the unit is the diameter of tree at breast height in inches - so, tree 199599 is recorded to have a diameter of 435 inches** 😱 **In addition, I am not sure how a tree can have a diameter of 0. Perhaps some quality control of the data needs to be done before further analyses should be done.**
...
2.  **Is there a relationship between the species of tree versus where and how it's planted (ex. with root barrier or not, on a curb or not, plant area)**
